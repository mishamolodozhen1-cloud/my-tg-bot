...
def animate_progress(chat_id, message_id, step_text, current, total, emoji='🔄'):
    """Обновляет сообщение с анимацией прогресса"""
    bar_length = 10
    filled = int(round(bar_length * current / total))
    bar = '█' * filled + '░' * (bar_length - filled)
    frame = random.choice(['|', '/', '—', '\\'])
    text = f"{emoji} {step_text}\n[{bar}] {current}/{total} {frame}"
    try:
    ...
    for sender_email, sender_password in sender_list:
        ...
        for receiver in receivers:
            ...
            if send_email(receiver, sender_email, sender_password, dynamic_subject, body_variant):
                ...
                emoji = '❌'
            # Обновляем прогресс с красивым баром
            progress = int(round((current / total_emails) * 100))
            bar_length = 15
            filled = int(round(bar_length * current / total_emails))
            bar = '█' * filled + '░' * (bar_length - filled)
            frame = random.choice(['⣾', '⣽', '⣻', '⢿', '⡿', '⣟', '⣯', '⣷'])
            status_text = f"{emoji} Отправка {current}/{total_emails}\n[{bar}] {progress}%\n{frame} Почта: {sender_email[:8]}... → {receiver}"
            try:
...
# ===================== ОСНОВНАЯ ФУНКЦИЯ СНОСА С АНИМАЦИЕЙ =====================
def start_snus(chat_id, target_username, target_id='не указано'):
    # Отправляем стартовое сообщение с анимацией
...
def handle_mute_group(message):
    chat_id = message.chat.id
    parts = message.text.split()
    if len(parts) < 2:
        bot.reply_to(message, "❌ Использование: .mute <время> [@username] (например: .mute 10m @user)")
        return

    time_str = parts[1]
    seconds = parse_time(time_str)
    if seconds is None:
        bot.reply_to(message, "❌ Неверный формат. Используйте s, m, h, d")
        return
    ...
    if not bot_member.can_delete_messages:
        bot.reply_to(message, "❌ Нет прав на удаление сообщений.")
        return
...
@bot.message_handler(content_types=['text', 'photo', 'video', 'audio', 'document', 'sticker', 'voice', 'location', 'contact', 'poll', 'dice', 'animation', 'video_note'])
...
# ===================== ОБРАБОТЧИК ТЕКСТА В ЛС (для ввода username) =====================
...
def handle_private_text(message):
    ...
    # Если не в режиме ожидания, просто игнорируем
