...
user_data = {}
cancel_flags = {}
bomber = SmsBomber()  # экземпляр бомбера
...
👑 Создатель: AlternativeHospital
        ...
        try:
            ...
            logger.error(f"Ошибка финала бомбера: {e}")
...
def handle_callback(call):
    ...
    if call.data == "menu_bomber":
        bot.send_message(chat_id, "📱 **Введите номер телефона** (в международном формате, например 79001234567):", parse_mode='Markdown')
...
# ===================== ОБРАБОТКА ТЕКСТОВЫХ СООБЩЕНИЙ (ВВОД USERNAME/ID/ТЕЛЕФОНА) =====================
...
def handle_private_text(message):
    ...
    if user_id in user_data:
        ...
        if step == 'awaiting_username':
            ...
            # Проверяем номер (только цифры, можно с +)
            ...
            if len(phone_clean) >= 10 and len(phone_clean) <= 15:
                ...
                bot.send_message(chat_id, "❌ Некорректный номер телефона. Введите от 10 до 15 цифр (с кодом страны).")
        else:
...
def run_bot():
    while True:
        try:
            logger.info("🚀 Бот запущен (снос + бомбер)...")
