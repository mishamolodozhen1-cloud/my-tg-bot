...
# ================== ИСПРАВЛЕННАЯ ФУНКЦИЯ ОТПРАВКИ (порт 587 + STARTTLS) ==================
def send_email(from_email: str, password: str, to_email: str, subject: str, body: str) -> bool:
    try:
        ...
        # Попытка через порт 587 с STARTTLS (предпочтительно для Railway)
        try:
            with smtplib.SMTP("smtp.mail.ru", 587, timeout=60) as smtp:
                smtp.starttls()
                smtp.login(from_email, password)
                smtp.send_message(msg)
            logging.info(f"✅ {from_email} -> {to_email} (порт 587)")
            return True
        except Exception as e:
            logging.warning(f"⚠️ Порт 587 не работает для {from_email}: {e}")
            # Запасной вариант: порт 465 SSL (если 587 заблокирован)
            with smtplib.SMTP_SSL("smtp.mail.ru", 465, timeout=60) as smtp:
                smtp.login(from_email, password)
                smtp.send_message(msg)
            logging.info(f"✅ {from_email} -> {to_email} (порт 465)")
            return True
    except Exception as e:
