...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "cancel":
        await query.edit_message_text("❌ Операция отменена.", reply_markup=None)
    ...
    if data == "stop_bomb":
        ...
        if task and not task.done():
            task.cancel()
            await query.edit_message_text("⏹ Бомбинг остановлен.", reply_markup=None)
    ...
    if data == "help":
        await query.edit_message_text(
            ...
            "2. Выберите длительность атаки.\n"
            "3. Введите номер телефона (только цифры).\n"
            "4. Наблюдайте за анимацией прогресса.\n"
            "5. В любой момент нажмите *Остановить*.\n\n"
            "Максимальное время – 10 минут (600 сек).",
            parse_mode="Markdown",
            reply_markup=MAIN_MENU
        )
        return ConversationHandler.END
    ...
    if data.startswith("time_"):
        ...
        return PHONE  # переходим в состояние ожидания номера

    return ConversationHandler.END

async def phone_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обрабатывает текстовое сообщение с номером телефона."""
...
async def bomb_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Скрытая команда /bomb – оставлена для обратной совместимости."""
    chat_id = update.effective_chat.id
    args = context.args
    if not args:
...
async def start_bombing_task(update_obj, context, phone, duration, chat_id):
    """Запускает асинхронную задачу бомбинга с живым обновлением."""
    if chat_id in active_tasks and not active_tasks[chat_id].done():
        active_tasks[chat_id].cancel()
        await asyncio.sleep(0.5)

    # Сообщение, которое будем обновлять
    if hasattr(update_obj, 'message'):
        msg = await update_obj.message.reply_text(
            "⏳ Инициализация...",
            reply_markup=STOP_KEYBOARD
        )
    else:
        # Если это callback, то update_obj - это CallbackQuery
        msg = await update_obj.edit_message_text(
            "⏳ Инициализация...",
            reply_markup=STOP_KEYBOARD
        )
...
async def cancel_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отмена диалога (если пользователь ввёл что-то не то)."""
...
def main():
    ...
    # ConversationHandler для ввода номера после выбора времени
