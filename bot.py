...
# ========== КОНФИГУРАЦИЯ ==========
BOT_TOKEN = "8523492309:AAF1z6AdIrG9sE1BzU_3p7AuRYedOU5edXs"
OPENROUTER_API_KEY = "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"  # ваш рабочий ключ

# ВЫБРАННЫЕ МОДЕЛИ
CHAT_MODEL = "nvidia/nemotron-3-ultra-550b-a55b:free"   # для текста
TTS_MODEL = "deepgram/flux-tts:free"                    # для озвучки
IMAGE_MODEL = "openrouter/free"                         # для изображений (автовыбор)

TTS_VOICE = "flux-alexis-en"   # голос для TTS (можно сменить на другой)
MAX_HISTORY = 10
...
def chat_with_ai(user_id, user_text):
    ...
    # Если история пуста – добавляем системный промпт на русском
    if not history:
        history.append({"role": "system", "content": "Отвечай на русском языке."})
    
    # Добавляем сообщение пользователя
    history.append({"role": "user", "content": user_text})
    
    # Ограничиваем длину истории
    if len(history) > MAX_HISTORY * 2 + 1:  # +1 для системного
        history = history[:1] + history[-(MAX_HISTORY * 2):]  # сохраняем системное
    ...
    headers = {
        ...
        "X-OpenRouter-Title": "Telegram Bot",
    }
    payload = {
        "model": CHAT_MODEL,
        "messages": history,
...
if __name__ == "__main__":
    print("✅ Бот запущен с моделями: чат – Nemotron 3 Ultra, TTS – Flux, изображения – openrouter/free.")
