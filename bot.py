...
def animate_loading(chat_id, message_id, text, frames=None):
    """Вращающаяся анимация с набором кадров"""
    if frames is None:
        frames = ['⣾', '⣽', '⣻', '⢿', '⡿', '⣟', '⣯', '⣷']
    for _ in range(4):
        for f in frames:
...
def animate_stage(chat_id, message_id, emoji, text, duration=1.2):
    """Этап с крупным эмодзи и текстом"""
    try:
...
def animate_progress(chat_id, message_id, current, total, step_text='🚀 Отправка', extra='', cancel_callback=None):
    """Прогресс-бар с меняющимися иконками и мерцанием"""
    bar_length = 25  # длиннее, красивее
    filled = int(round(bar_length * current / total))
    # Создаём градиентный бар (меняем цвет символов)
    bar_parts = []
    for i in range(bar_length):
        if i < filled:
            # Используем разные символы для эффекта градиента
            if i < bar_length // 3:
                bar_parts.append('🟥')
    ...
    # Добавляем случайное мерцание (звёздочки)
...
⚡ **Как использовать:**
- Нажми кнопку «🧑 Снос аккаунта» ниже.
- Введи username (без @).
- Затем введи Telegram ID (число).
...
def run_bot():
    while True:
        try:
            logger.info("🚀 Бот запущен (красивая анимация)...")
