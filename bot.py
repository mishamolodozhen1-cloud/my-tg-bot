...
def animate_loading(chat_id, message_id, text, frames=None):
    """Вращающаяся анимация с выбором кадров"""
    if frames is None:
        frames = ['⣾', '⣽', '⣻', '⢿', '⡿', '⣟', '⣯', '⣷']
    for _ in range(4):  # 4 цикла для плавности
        for f in frames:
            try:
...
def animate_stage(chat_id, message_id, emoji, text, duration=2.0):
    """Показывает статичный этап с эмодзи на время"""
    try:
...
def animate_progress(chat_id, message_id, current, total, step_text='Отправка', extra=''):
    """Прогресс-бар с меняющимися иконками в зависимости от прогресса"""
    bar_length = 20
...
def send_complaints_batch(complaint_text, subject, email_list, chat_id, status_msg_id):
    ...
    # Этап 1: подготовка
    animate_stage(chat_id, status_msg_id, '🔌', 'Подключение к почтовым ящикам...', 1.5)
    animate_loading(chat_id, status_msg_id, '📡 Синхронизация SMTP...', frames=['◐','◓','◑','◒'])
    animate_stage(chat_id, status_msg_id, '✅', 'Готово! Начинаю рассылку...', 1.0)
    ...
    for sender_email, sender_password in sender_list:
        ...
        for receiver in receivers:
            ...
            success = send_email(receiver, sender_email, sender_password, dynamic_subject, body_variant)
            if success:
                total_sent += 1
                status = '✅'
            else:
                status = '❌'
            # Обновляем прогресс с дополнительной информацией
            extra = f"{status} {sender_email[:8]}... → {receiver[:12]}"
            animate_progress(chat_id, status_msg_id, current, total_emails, '🚀 Отправка', extra)
...
def start_snus(chat_id, target_username, target_id='не указано'):
    ...
    time.sleep(0.5)
    # Анимированная подготовка
    animate_loading(chat_id, start_msg.message_id, "🔄 Активация двигателей...", frames=['◐','◓','◑','◒'])
    animate_stage(chat_id, start_msg.message_id, '⚡', 'Заряжаем ядерный заряд...', 1.2)
    animate_loading(chat_id, start_msg.message_id, '🔥', frames=['▰','▱','▰','▱'])
    
    complaint = generate_hard_complaint(target_username, target_id)
    subject = "СРОЧНАЯ ЖАЛОБА на пользователя Telegram"

    # Создаём новое сообщение для прогресса
...
💣 *Ожидайте, скоро аккаунт исчезнет навсегда.*

👑 Создатель: AlternativeHospital
        """
        try:
            # Эффект фейерверка: несколько обновлений с разными эмодзи
            for _ in range(3):
...
⚡ **Как использовать:**
- Напиши мне в личку команду `.snos <username>` (без @)
- Или нажми кнопку «Снос аккаунта» и введи username

💣 **Результат:** аккаунт получает **максимальное количество жалоб** от 13 почтовых ящиков на все адреса поддержки Telegram. Блокировка неизбежна!
...
def handle_snos(message):
    ...
    if len(parts) < 2:
        bot.reply_to(message, "❌ Использование: `.snos <username>` (без @)", parse_mode='Markdown')
        return
...
def handle_private_text(message):
    ...
    if user_id in user_data and user_data[user_id].get('step') == 'awaiting_username':
        ...
        return
    bot.send_message(chat_id, "Используйте /start для меню или `.snos username` для быстрого сноса.", parse_mode='Markdown')

# ===================== ЗАПУСК =====================
def run_bot():
    while True:
        try:
            logger.info("🚀 Бот запущен (супер-анимация)...")
