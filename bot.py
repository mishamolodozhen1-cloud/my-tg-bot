...
# ---------- БАЗА ДАННЫХ (ПОЛНАЯ ИНИЦИАЛИЗАЦИЯ) ----------
DB_FILE = "game.db"
...
def init_db():
    ...
    cur.execute("""
        CREATE TABLE IF NOT EXISTS technologies (user_id INTEGER, tech_name TEXT, level INTEGER DEFAULT 0, last_upgrade TIMESTAMP, PRIMARY KEY (user_id, tech_name))
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS research_queue (user_id INTEGER, tech_name TEXT, start_time TIMESTAMP, finish_time TIMESTAMP, PRIMARY KEY (user_id, tech_name))
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS spy_operations (id INTEGER PRIMARY KEY AUTOINCREMENT, from_user INTEGER, target_user INTEGER, action TEXT, date TIMESTAMP, success INTEGER)
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS events_log (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, event_type TEXT, description TEXT, date TIMESTAMP)
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS treaties (user_id INTEGER, partner_id INTEGER, type TEXT, until TIMESTAMP, PRIMARY KEY (user_id, partner_id))
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS colonies (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, name TEXT, income INTEGER DEFAULT 10, loyalty INTEGER DEFAULT 50, invested INTEGER DEFAULT 0, last_update TIMESTAMP)
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS trades (id INTEGER PRIMARY KEY AUTOINCREMENT, from_user INTEGER, to_user INTEGER, status TEXT, offered_type TEXT, offered_amount INTEGER, requested_type TEXT, requested_amount INTEGER, date TIMESTAMP)
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS allies (user_id INTEGER, ally_id INTEGER, status TEXT, date TIMESTAMP, PRIMARY KEY (user_id, ally_id))
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS sanctions (from_user INTEGER, target_user INTEGER, date TIMESTAMP, PRIMARY KEY (from_user, target_user))
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS wars (id INTEGER PRIMARY KEY AUTOINCREMENT, start_time TIMESTAMP, status TEXT, winner_id INTEGER DEFAULT NULL, initiator_id INTEGER, target_id INTEGER)
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS war_participants (war_id INTEGER, user_id INTEGER, side TEXT, joined_at TIMESTAMP, PRIMARY KEY (war_id, user_id))
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS peace_requests (war_id INTEGER, from_user INTEGER, to_user INTEGER, status TEXT, date TIMESTAMP, PRIMARY KEY (war_id, from_user, to_user))
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS currency_rates (currency TEXT PRIMARY KEY, rate REAL DEFAULT 1.0, last_update TIMESTAMP)
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS votes (id INTEGER PRIMARY KEY AUTOINCREMENT, proposer_id INTEGER, proposal TEXT, description TEXT, status TEXT, until TIMESTAMP, votes_for INTEGER DEFAULT 0, votes_against INTEGER DEFAULT 0)
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS vote_records (vote_id INTEGER, user_id INTEGER, choice TEXT, PRIMARY KEY (vote_id, user_id))
    """)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS occupations (occupier_id INTEGER, occupied_country TEXT, since TIMESTAMP, resistance INTEGER DEFAULT 0, PRIMARY KEY (occupier_id, occupied_country))
    """)
    conn.commit()
    conn.close()

# ---------- ДАННЫЕ СТРАН (ПОЛНЫЙ СПИСОК) ----------
COUNTRIES_DATA = {
...
# ---------- ГЛАВНОЕ МЕНЮ В ГРУППЕ ----------
async def main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обработчик /start только для групп."""
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_mystats":
        ...
        await handle_submenu_action(update, context, data)

# ---------- ПОДМЕНЮ (РЕАЛИЗОВАНЫ) ----------
async def show_mystats(update, context):
    query = update.callback_query
    user_id = query.from_user.id
    stats = get_user_stats(user_id)
    if not stats:
        await query.edit_message_text("❌ Вы не выбрали страну.")
        return
...
async def show_war_menu(update, context):
    query = update.callback_query
    keyboard = [
        [InlineKeyboardButton("⚔️ Объявить войну", callback_data="war_declare")],
        [InlineKeyboardButton("🕊️ Предложить мир", callback_data="war_peace")],
        [InlineKeyboardButton("⚔️ Вступить в войну", callback_data="war_join")],
        [InlineKeyboardButton("⏹️ Завершить войну", callback_data="war_end")],
        [InlineKeyboardButton("↩️ Назад", callback_data="menu_back")]
    ]
    await query.edit_message_text("⚔️ Меню войны:", reply_markup=InlineKeyboardMarkup(keyboard))

async def show_diplomacy_menu(update, context):
    query = update.callback_query
    keyboard = [
        [InlineKeyboardButton("🤝 Предложить союз", callback_data="diplo_ally")],
        [InlineKeyboardButton("💔 Разорвать союз", callback_data="diplo_break_ally")],
        [InlineKeyboardButton("📜 Договор о ненападении", callback_data="diplo_napt")],
        [InlineKeyboardButton("🚫 Ввести санкции", callback_data="diplo_sanction")],
        [InlineKeyboardButton("✅ Снять санкции", callback_data="diplo_remove_sanction")],
        [InlineKeyboardButton("↩️ Назад", callback_data="menu_back")]
    ]
    await query.edit_message_text("🤝 Дипломатия:", reply_markup=InlineKeyboardMarkup(keyboard))

async def show_economy_menu(update, context):
    query = update.callback_query
    keyboard = [
        [InlineKeyboardButton("💰 Инвестировать", callback_data="eco_invest")],
        [InlineKeyboardButton("⚔️ Мобилизация", callback_data="eco_mobilize")],
        [InlineKeyboardButton("🛡️ Демобилизация", callback_data="eco_demobilize")],
        [InlineKeyboardButton("🏰 Укрепления", callback_data="eco_fortify")],
        [InlineKeyboardButton("📉 Кризис", callback_data="eco_crisis")],
        [InlineKeyboardButton("💱 Курс валюты", callback_data="eco_currency")],
        [InlineKeyboardButton("↩️ Назад", callback_data="menu_back")]
    ]
    await query.edit_message_text("💰 Экономика:", reply_markup=InlineKeyboardMarkup(keyboard))

async def show_research_menu(update, context):
    query = update.callback_query
    user_id = query.from_user.id
    keyboard = []
    ...
    keyboard.append([InlineKeyboardButton("↩️ Назад", callback_data="menu_back")])
    await query.edit_message_text("🔬 Технологии:", reply_markup=InlineKeyboardMarkup(keyboard))

async def show_spy_menu(update, context):
    query = update.callback_query
    keyboard = [
        [InlineKeyboardButton("🔍 Разведка", callback_data="spy_recon")],
        [InlineKeyboardButton("💥 Саботаж", callback_data="spy_sabotage")],
        [InlineKeyboardButton("↩️ Назад", callback_data="menu_back")]
    ]
    await query.edit_message_text("🕵️ Шпионаж:", reply_markup=InlineKeyboardMarkup(keyboard))

async def show_colonies_menu(update, context):
    query = update.callback_query
    keyboard = [
        [InlineKeyboardButton("🌍 Список колоний", callback_data="col_list")],
        [InlineKeyboardButton("💰 Инвестировать в колонию", callback_data="col_invest")],
        [InlineKeyboardButton("↩️ Назад", callback_data="menu_back")]
    ]
    await query.edit_message_text("🌍 Колонии:", reply_markup=InlineKeyboardMarkup(keyboard))

async def show_fleet_menu(update, context):
    query = update.callback_query
    keyboard = [
        [InlineKeyboardButton("⚓ Построить флот", callback_data="fleet_build")],
        [InlineKeyboardButton("🚫 Морская блокада", callback_data="fleet_blockade")],
        [InlineKeyboardButton("↩️ Назад", callback_data="menu_back")]
    ]
    await query.edit_message_text("⚓ Флот:", reply_markup=InlineKeyboardMarkup(keyboard))

async def show_vote_menu(update, context):
    query = update.callback_query
    keyboard = [
        [InlineKeyboardButton("🗳️ Создать голосование", callback_data="vote_create")],
        [InlineKeyboardButton("🗳️ Проголосовать", callback_data="vote_cast")],
        [InlineKeyboardButton("↩️ Назад", callback_data="menu_back")]
    ]
    await query.edit_message_text("🗳️ Голосования:", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def show_season(update, context):
    ...
    if 12 <= month <= 2:
        ...
        season = '🍂 Осень'
    keyboard = [[InlineKeyboardButton("↩️ Назад", callback_data="menu_back")]]
    await query.edit_message_text(f"🧭 Текущий сезон: {season}", reply_markup=InlineKeyboardMarkup(keyboard))

async def show_bots(update, context):
    query = update.callback_query
    bot_ids = get_bot_ids()
    if not bot_ids:
        await query.edit_message_text("Нет ботов.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("↩️ Назад", callback_data="menu_back")]]))
        return
...
# ---------- ОБРАБОТЧИК SUBMENU ACTION (ЗАГЛУШКИ ДЛЯ ПРИМЕРА) ----------
async def handle_submenu_action(update, context, data):
    query = update.callback_query
    # В реальном коде здесь будут полноценные обработчики для всех действий
    # Например, war_declare → список стран для войны и т.д.
    await query.edit_message_text("Функция в разработке. Нажмите 'Назад'.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("↩️ Назад", callback_data="menu_back")]]))

# ---------- ПРИВЕТСТВИЕ ПРИ ДОБАВЛЕНИИ В ГРУППУ (С ПИНОМ) ----------
...
def main():
    ...
    # Обработчик /start (только для групп, в личке — отказ)
